    ...
    return (
        <div className='flex items-center justify-center flex-col gap-4 mt-12 mb-12 pl-4 pr-4 w-full'>
            <div className='flex flex-col md:flex-row w-full gap-4'>
                <div className='w-full md:w-96 flex-1 h-32'>
                    <ForecastCard weatherForecast={weatherForecast} />
                </div>
                <div className='h-32 w-32 hidden md:flex'><WidgetCard TitleIcon={Thermometer} title={"Sensação"} content={`${Math.ceil(weatherData.main.feels_like)}°`} footer={`A humidade pode afetar a sensação térmica`} /></div>
            </div>
            <div className='flex flex-col md:flex-row w-full gap-4'>
                <div className='w-full'><WeatherPrevisionCard weatherForecast={weatherForecast} /></div>
